...
public class ViewManager
{
	        ...
	        throws Exception
	{
		...
		stage.getScene().getStylesheets().add(MagicValues.MainStyleFile.toURI().toString());
	...
	public static void closeParentStage(final Node element)
	{
		...
		stage.close();
		if (windows.contains(getWindow(((ControllerBase) loader.getController()).getName())))
	        ...
	        final EventHandler<WindowEvent> event, final GenericData data)
	{
		try
		{
			...
			stage.getIcons().add(MagicValues.ApplicationIcon);
			stage.setScene(new Scene(loader.load()));
			stage.getScene().getStylesheets().add(MagicValues.MainStyleFile.toURI().toString());
			stage.setUserData(loader);
