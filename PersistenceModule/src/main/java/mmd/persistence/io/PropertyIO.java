...
public class PropertyIO
{
	 ...
	 */
	public static void addDMDefinitionToFile(final DMBase<?> dm, final String filePath,
	...
	public static <T> List<T> getDMDefinitionsFromFile(final String filePath, final Class<?> type)
	{
		...
		try
		{
			...
			NodeList childNodes = root
			        .getElementsByTagName(((DMDefinition<T>) type.newInstance()).getName());
			
			for (int i = 0; i < childNodes.getLength(); i++)
			{
				T object = ((DMDefinition<T>) type.newInstance())
				        .newInstance(Property.getObjectFromProperties(
				                getPropertyListFromNode(childNodes.item(i), type)));
	 ...
	 */
	public static void removeDMDefinitionFromFile(final DMBase<?> dm, Field field,
	        ...
	        final List<Property> children)
	{
		...
		try
		{
			...
			for (Property child : children)
			{
				...
				{
					element = getElementFromPropertyList(root, child.getName(),
					        child.getChildren());
				}
	...
	public static void updateDMDefinitionFromFile(DMBase dm, DMBase newdm, Field field,
	        String filePath) throws Exception
	{
		...
		Element newnode = getElementFromPropertyList(document, node.getNodeName(),
		        newdm.getProperties());
